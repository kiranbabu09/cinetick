...
function UserProfile() {
  ...
  return (
    <Box width={"100%"} display={"flex"}> <Fragment>
      ...
      <Typography padding={1} width={"auto"} textAlign={"center"} border={"1px solid #ccc"} borderRadius={6}>Name:<b>{user.name}</b></Typography>
      <Typography mt={2} padding={1} width={"auto"} textAlign={"center"} border={"1px solid #ccc"} borderRadius={6}>Email:<b>{user.email}</b></Typography>
      </Box>)}
      {bookings && 
          ...
          <List>
            {bookings.map((item,index)=>
              <ListItem key={index} sx={{bgcolor:"#00d386",color:"white",margin:1,display:"flex",justifyContent:"flex-start"}}>
                <ListItemText key={"movie"} sx={{margin:1,width:"40%",textAlign:"left"}} ><b>Movie: {item.movie.title}</b></ListItemText>
                <ListItemText key={"seat"} sx={{margin:1,width:"15%",textAlign:"left"}} ><b>Seat: {item.seatNumber}</b></ListItemText>
                <ListItemText key={"date"} sx={{margin:1,width:"30%",textAlign:"left"}} ><b>Date: {new Date(item.date).toDateString()}</b></ListItemText>
